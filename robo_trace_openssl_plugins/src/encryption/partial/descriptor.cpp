...
OpenSSLPartialEncryptionStageDescriptor::OpenSSLPartialEncryptionStageDescriptor(const OpenSSLPluginKeyManager::Ptr& key_manager, const ros::NodeHandle& plugin_namespace)
: OpenSSLPartialEncryptionStageDescriptor(key_manager, plugin_namespace, std::make_shared<ros_babel_fish::IntegratedDescriptionProvider>()) {
    //
}

OpenSSLPartialEncryptionStageDescriptor::OpenSSLPartialEncryptionStageDescriptor(const OpenSSLPluginKeyManager::Ptr& key_manager, const ros::NodeHandle& plugin_namespace, const ros_babel_fish::DescriptionProvider::Ptr message_description_provider) 
: ProcessingStageDescriptor(plugin_namespace, "openssl_partial_encryption"), m_key_manager(key_manager), m_message_description_provider(message_description_provider) {
...
bool OpenSSLPartialEncryptionStageDescriptor::isModeSupported(const ProcessingStage::Mode mode) const {
    // Currently only FORWARD is implemented.
    return mode == ProcessingStage::Mode::FORWARD;
}

bool OpenSSLPartialEncryptionStageDescriptor::isExclusiveToTopic(const ProcessingStage::Mode mode) const {
    return true;
}

bool OpenSSLPartialEncryptionStageDescriptor::isConcurrentlyExecutable(const ProcessingStage::Mode mode) const {
    return false;
}


const OpenSSLPluginKeyManager::Ptr& OpenSSLPartialEncryptionStageDescriptor::getKeyManager() const {
    return m_key_manager;
}

const OpenSSLPartialEncryptionConfiguration::Ptr& OpenSSLPartialEncryptionStageDescriptor::getConfiguration() const {
    return m_configuration;
}


ProcessingStage::Ptr OpenSSLPartialEncryptionStageDescriptor::getStage(const ProcessingStage::Mode mode, const std::string& topic, const std::string& message_type) {
    switch(mode) {

        case ProcessingStage::Mode::FORWARD : 
            return std::make_shared<OpenSSLPartialEncryptionProcessingStage>(m_configuration, m_key_manager, m_message_description_provider, message_type);
